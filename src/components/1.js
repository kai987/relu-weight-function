...
const WeightLearningExperiment = () => {
	...
	const runExperiment = () => {
		const method = parameters.method === 'relu' ? runReLUMethod : runWeightMethod;
		const data = method(parameters);
		setResults(data);
	};
	...
	// 複数の学習率での比較実験
	const compareAlphaValues = () => {
		const alphas = [0.001, 0.01, 0.1, 0.5];
		const comparison = alphas.map(alpha => {
			const testParams = { ...parameters, alpha };
			const method = parameters.method === 'relu' ? runReLUMethod : runWeightMethod;
			const data = method(testParams);
			return {
				alpha,
				finalError: Math.abs(data[data.length - 1].error),
				converged: Math.abs(data[data.length - 1].error) < 0.1,
				oscillated: data.some((d, i) => i > 0 && Math.abs(d.error) > Math.abs(data[i-1].error))
			};
		});
		return comparison;
	};

	const alphaComparison = compareAlphaValues();
				...
				<div className={styles.parameterGrid}>
					...
					<div className={styles.inputGroup}>
						...
						<input
							type="number"
							value={parameters.x2}
							onChange={(e) => handleParameterChange('x2', e.target.value)}
							className={styles.input}
						/>
					</div>
					<div className={styles.inputGroup}>
					...
					<div className={styles.inputGroup}>
						...
						<input
							type="number"
							value={parameters.w1}
							onChange={(e) => handleParameterChange('w1', e.target.value)}
							className={styles.input}
						/>
					</div>
					<div className={styles.inputGroup}>
						...
						<input
							type="number"
							value={parameters.w2}
							onChange={(e) => handleParameterChange('w2', e.target.value)}
							className={styles.input}
						/>
					</div>
					<div className={styles.inputGroup}>
			...
			<div className={styles.chartSection}>
				...
				<div className={styles.tableContainer}>
					<table className={styles.table}>
						...
						<tbody>
						{alphaComparison.map((result, index) => (
                    ...
                    <span className={`${styles.badge} ${result.converged ? styles.badgeSuccess : styles.badgeError}`}>
                      {result.converged ? '収束' : '未収束'}
                    </span>
								</td>
								<td className={`${styles.tableCell} ${styles.tableCellLast}`}>
                    <span className={`${styles.badge} ${result.oscillated ? styles.badgeWarning : styles.badgeInfo}`}>
